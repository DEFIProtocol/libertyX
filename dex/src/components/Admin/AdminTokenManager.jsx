// AdminTokenManager.jsx - Optimized version with JSON as default, DB for comparison
        ...
        <>
            <div className="manager-header">
                <div className="header-left">
                    <h2>Token Management {usingJson ? '(JSON Default)' : '(DB Fallback)'}</h2>
                    <div className="data-source-info">
                        <span className={`json-count ${errorJson ? 'error' : ''}`}>
                            📄 JSON: {loadingJson ? '...' : errorJson ? '❌' : jsonCount} {usingJson && '⭐'}
                        </span>
                        <span className={`db-count ${errorDb ? 'error' : ''}`}>
                            🛢️ DB: {loadingDb ? '...' : errorDb ? '❌' : dbCount}
                        </span>
                        <span className="total-count">
                            📊 Showing: {filteredAndSortedTokens.length} / {displayTokensCount}
                        </span>
                    </div>
                </div>
...
const ComparisonView = React.memo(function ComparisonView({
    viewMode,
    setViewMode,
    comparisonData, // Changed from 'tokens' to 'comparisonData'
    selectedTokens,
    onSelectToken,
    expandedRows,
    onToggleExpansion,
    ...
    const filteredTokens = useMemo(() => {
        ...
        switch(viewMode) {
            case 'onlyDb':
                return comparisonData.filter(item => item.inDatabase && !item.inJson);
            case 'onlyJson':
                return comparisonData.filter(item => !item.inDatabase && item.inJson);
            case 'diff':
                return comparisonData.filter(item => item.inDatabase && item.inJson && !item.match);
            case 'both':
                return comparisonData.filter(item => item.inDatabase && item.inJson);
            default:
...
const AdminTokenManager = React.memo(function AdminTokenManager({ tokens: tokensProp = [], isLoading = false }) {
    const { 
        comparisonMode, 
        toggleComparisonMode,
        loadingAll,
        ...
        jsonCount,
        dbTokens,
        jsonTokens,  // Make sure to import jsonTokens
        refreshAll
    } = useTokens();
    ...
    // 🎯 FALLBACK LOGIC: Use JSON tokens by default, fall back to DB if JSON is empty
    const displayTokens = useMemo(() => {
        // First priority: JSON tokens (from file)
        if (Array.isArray(jsonTokens) && jsonTokens.length > 0) {
            console.log('📄 Admin: Using JSON tokens as default:', jsonTokens.length);
            return jsonTokens;
        }
        
        // Second priority: tokensProp if provided and JSON is empty
        if (Array.isArray(tokensProp) && tokensProp.length > 0) {
            console.log('📋 Admin: Using tokens from props:', tokensProp.length);
            return tokensProp;
        }
        
        // Third priority: DB tokens as fallback
        if (Array.isArray(dbTokens) && dbTokens.length > 0) {
            console.log('🛢️ Admin: Falling back to DB tokens:', dbTokens.length);
            return dbTokens;
        }
        
        // Last resort: empty array
        console.log('⚠️ Admin: No tokens available');
        return [];
    }, [jsonTokens, tokensProp, dbTokens]);
    ...
    const filteredAndSortedTokens = useMemo(() => {
        ...
        if (debouncedSearchTerm) {
            ...
            filtered = [];
            for (let i = 0; i < Math.min(displayTokens.length, 1000); i++) {
            ...
            filtered = displayTokens.slice(0, 200);
        }
        ...
        return filtered;
    }, [displayTokens, debouncedSearchTerm, sortConfig, getSortValue]);
    ...
    }, [createToken, updateToken, oneInchBySymbol, rapidBySymbol, refreshAll, dbTokenBySymbol, oneInchChainKey]);
    ...
    }, [dbTokens, rapidBySymbol, deleteToken, refreshAll]);

    if (loadingAll && !displayTokens.length) {
    ...
    return (
        <div className="admin-token-manager">
            <div className="data-source-badge">
                {jsonTokens.length > 0 ? '📄 Using JSON as default' : '🛢️ Using DB as fallback'}
                {dbTokens.length > 0 && jsonTokens.length > 0 && (
                    <span className="source-stats">
                        (JSON: {jsonTokens.length} | DB: {dbTokens.length})
                    </span>
                )}
            </div>
            ...
            {comparisonMode ? (
                <ComparisonView 
                    ...
                    comparisonData={tokensProp} // Pass tokensProp as comparisonData
                    selectedTokens={selectedTokens}
                ...
                <NormalView 
                    ...
                    displayTokensCount={displayTokens.length}
                    usingJson={jsonTokens.length > 0}
                />
            )}
        </div>
    );
